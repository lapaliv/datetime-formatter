...
describe('formatter', () => {
  ...
  describe('format', () => {
    ...
    describe('D', () => {
      ...
      const dates = {
        '2021-01-01': shortDayNames[4],
        '2020-10-03': shortDayNames[5],
        '2021-06-15': shortDayNames[1],
      };

      for (const date in dates) {
        if (Object.hasOwnProperty.call(dates, date)) {
          it(`D (${date})`, () => {
            const formatter = DateTimeFormatter.parse(date);
            expect(formatter.format('D')).toBe((dates as any)[date]);
          })
        }
      }
    });
